...
<c:if test="${empty list}">
	 ...
	 <h3 class="text-center default-text text-dark py-3"><s:message code="label.courselist" /></h3>
    ...
    <button type="submit" class="btn btn-warning " name="operation" value="Reset" ><s:message code="label.reset"/></button> 
      ...
      <th class="text-center"><input type="checkbox" id="select_all" name="select"><s:message code="label.selectall"/></th>
                   <th><s:message code="label.SNo"/></th>
					...
					<th><s:message code="label.duration"/></th>
               ...
               <tbody>
                  <c:forEach items="${list}" var="course" varStatus="ct">
					<td class="text-center"><input type="checkbox" class="checkbox" name="ids" value="${course.id}"></td>
					<td>
					...
					<td><c:url var="editUrl" value="/ctl/Course/AddCourse?id=" />
              			<a class="btn btn-outline-success btn-block btn-round w-50" style="border-color: #8A2BE2; color: #8A2BE2;" href="${editUrl}${course.id}"><i class="fa fa-edit" aria-hidden="true"></i><s:message code="label.edit"/></a>
									</td>
					</tr>
				</c:forEach>
				</tbody>
