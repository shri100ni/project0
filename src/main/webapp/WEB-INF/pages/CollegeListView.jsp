...
<c:if test="${empty list}">
	    ...
	    <h3 class="text-center default-text text-dark py-3"><s:message code="label.collegelist" /></h3>
	 
	 <br>
	  <div class="row justify-content-center">
	  <div class="col-sm-2"></div>
	  <%-- <label class="form-check-label" for="check2" style = "width:50px">
      City:<s:message code="label.collegelist" />
      </label>
	  <div class="col-sm-2"><input placeholder="Subject Name" type="text" id="defaultForm-email"  name="City" class="border" style = "width:215px"  value="<%=ServletUtility.getParameter("City", request)%>">
</div>&ensp; --%>
	 <label class="form-check-label" for="check2" style = "width:50px">
      ...
      <th class="text-center"><input type="checkbox" id="select_all" name="select"><s:message code="label.selectall"/></th>
               ...
               <tbody>
				<c:forEach items="${list}" var="college" varStatus="ct">
					<td class="text-center"><input type="checkbox" class="checkbox" name="ids" value="${college.id}"></td>
					...
					<td>${college.phoneNo}</td>
										...
										<c:url var="editUrl" value="/ctl/College/AddCollege?id=" />
              							<a class="btn btn-outline-success btn-block btn-round w-50" style="border-color: #8A2BE2; color: #8A2BE2;" href="${editUrl}${college.id}"><i class="fa fa-edit" aria-hidden="true"></i><s:message code="label.edit"/></a>
									</td>
				</tr>
				</c:forEach>
				</tbody>
