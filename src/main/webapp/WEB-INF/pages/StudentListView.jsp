...
<c:if test="${empty list}">
	 ...
	 <h3 class="text-center default-text text-dark py-3"> <s:message code="label.studentlist" /></h3>
	 ...
	 <label class="form-check-label" for="check2" style = "width:50px">
	  ...
	  <div class="col-sm-2"> <sf:select path="collegeId"
  ...
  <button type="submit" class="btn btn-warning " name="operation" value="Reset" ><s:message code="label.reset"/></button> 
      ...
      <th class="text-center"><input type="checkbox" id="select_all" name="select"><s:message code="label.selectall"/></th>
               ...
               <c:forEach items="${list}" var="student" varStatus="ct">
					<td class="text-center"><input type="checkbox" class="checkbox" name="ids" value="${student.id}"></td>
					...
					<td><fmt:formatDate pattern="dd/MM/yyyy" value="${student.dob}" /></td>
					<c:forEach items="${collegeList}" var="clg">
												<c:if test="${student.collegeId==clg.id}">
					<td>${clg.name}</td>
					</c:if>
					</c:forEach>
					<td class="text-center"><c:url var="editUrl" value="/ctl/Student/AddStudent?id=" />
					 <a	class="btn btn-outline-success btn-block btn-round w-50" style="border-color: #8A2BE2; color: #8A2BE2;" name="operation"
												href="${editUrl}${student.id}"><i class="fa fa-edit" aria-hidden="true"></i>
