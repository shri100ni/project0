...
<c:if test="${empty list}">
     ...
     <s:message code="label.name" />
	  ...
	  <div class="col-sm-2"><sf:input path="login"
								cssClass="form-control border-left-right"
	 ...
	 <%-- <label class="form-check-label" for="check2" style = "width:50px">
      ...
      <th class="text-center"><input type="checkbox" id="select_all" name="select"><s:message code="label.selectall"/></th>
               ...
               <c:forEach items="${list}" var="user" varStatus="ct">
					<td class="text-center"><input type="checkbox" class="checkbox" name="ids" value=<c:choose>
					...
					<td>${user.mobileNo}</td>
					<td>${user.gender}</td>
					<td><fmt:formatDate	value="${user.dob}" type="date" pattern="dd-MM-yyyy" /></td>					
					<c:forEach items="${roleList}" var="role">
												<c:if test="${user.roleId==role.id}">
					<td>${role.name}</td>
					</c:if>
					</c:forEach>
<td ><c:url var="editUrl" value="/ctl/User/AddUser?id=" /> <c:choose>
													<c:when
														test="${user.roleId != 1 && sessionScope.user.id != user.id}">
														<a
															class="btn btn-outline-primary btn-block btn-round w-50"
													...
													<c:otherwise>
														<button
															class="btn btn-outline-secondary btn-block btn-round w-50"
															disabled>
