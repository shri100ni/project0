...
<c:if test="${empty list}">
	 ...
	 <h3 class="text-center default-text text-dark py-3"> <s:message code="label.rolelist"></s:message></h3>
      ...
      <c:set value="${((form.pageNo-1)*form.pageSize)+1}" var="index"></c:set>
               ...
               <tbody>
				<c:forEach items="${list}" var="role" varStatus="ct">
					<td class="text-center"><input type="checkbox" class="checkbox" name="ids" value="${role.id}"></td>
					<td class="">
...
<%-- 										<c:set var="index" value="${(form.pageSize * (form.pageNo-1))+ct.index+1}" />
 --%>										
					...
					<td><c:url var="editUrl" value="/ctl/Role/AddRole?id=" />
              			<a class="btn btn-outline-success btn-block btn-round w-50" style="border-color: #8A2BE2; color: #8A2BE2;" name="operation" href="${editUrl}${role.id}"><i class="fa fa-edit" aria-hidden="true"></i><s:message code="label.edit"/></a>
					</td>
				</tr>
				</c:forEach>
				</tbody>
